...
# Output: ""
#
# This is quite difficult problem in fact and you will be very unlucky if you have this on real interview.
# However if you familiar with Rabin-Karp algorighm, it will be not so difficult.
# You can see it in full details on Wikipedia: https://en.wikipedia.org/wiki/Rabin–Karp_algorithm
#
# Here I briefly explain the idea of it. Imagine, that we have string abcabcddcb, and we want to check if
# there are two substrings of size 3, which are equal. Idea is to hash this substrings, using rolling hash,
# where d is our base and q is used to avoid overflow.
#
...
# ...
# However, it can happen that we have collisions, we can falsely get two substrings with the same hash,
# which are not equal. So, we need to check our candidates.
#
# Binary search for help
# Note, that we are asked for the longest duplicate substring. If we found duplicate substring of length 10,
# it means that there are duplicate substrings of lenths 9,8, .... So, we can use binary search to find the longest one.
#
# Complexity of algorighm is O(n log n) if we assume that probability of collision is low.
#
# How to read the code
# I have RabinKarp(text, M,q) function, where text is the string where we search patterns,
# M is the length we are looking for and q is prime modulo for Rabin-Karp algorighm.
#
...
# Finally, we iterate over our dictionary and for each unique hash we check all possible combinations
# and compare not hashes, but original windows to make sure that it was not a collision.
# Now, to the longestDupSubstring(S) function we run binary search, where we run our
# RabinKarp funcion at most log n times.
import collections
